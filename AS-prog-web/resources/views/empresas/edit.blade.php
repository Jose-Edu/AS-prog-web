@extends('layouts.base')
	...
	@section('content')
		<form class="max-w-sm mx-auto" action="{{ url('empresas/'.$empresa->id) }}" method="POST">
			@csrf
			@method('PUT')

			<h1 class="self-center text-2xl font-semibold whitespace-nowrap dark:text-white mb-5">Editar Empresa:</h1>
			<div class="mb-5">
				<input type="text" id="nome" name="nome" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Nome" value="{{ $empresa->nome }}" required />
			</div>
